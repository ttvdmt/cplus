        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseMoved) {
                ...
                if (isSelecting && !isMoving) {
                    ...
                    for (Ball& b : balls)
                    {
                        if ((b.position.x >= mousePosition.x && b.position.x <= selectionRect.getPosition().x) ||
                            (b.position.x <= mousePosition.x && b.position.x >= selectionRect.getPosition().x))
                        {
                            if ((b.position.y >= mousePosition.y && b.position.y <= selectionRect.getPosition().y) ||
                                (b.position.y <= mousePosition.y && b.position.y >= selectionRect.getPosition().y))
                                b.isChoosen = true;
                        }
                        else
                            b.isChoosen = false;
                ...
                if (isMoving)
                {
                    ...
                    {
                        if (b.isChoosen)
                            b.position = mousePosition - offset;
                    }
                }
            }
                ...
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    for (Ball& b : balls) {
                        if (distance(mousePosition, b.position) < b.radius) {
                            isMoving = true;
                            offset = mousePosition - b.position;
                            b.isChoosen = true;
                            break;
                        }
                    }
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
                for (Ball& b : balls) {
                    if (b.isChoosen) {
                        b.color = sf::Color(rand() % 255, rand() % 255, rand() % 255, rand() % 255);
                        b.draw(window);
                    }
                } 
            }
